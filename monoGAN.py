...
parser.add_argument("--max_steps", type=int, default=44000,help="number of training steps (0 to disable)")
...
Examples = collections.namedtuple("Examples", "pathsL, pathsD, inputs, targets, count, steps_per_epoch")
...
def check_image(image):
    # done
    #image size [number of image, height, weight, channels=1]
    assertion = tf.assert_equal(tf.shape(image)[-1], 1, message="image must have 1 channel")
...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        ...
        a_images = preprocess(raw_input_L)
        b_images = preprocess(raw_input_depth)

    input_images, target_images = [a_images, b_images]

    paths_L_batch, paths_depth_batch, inputs_batch, targets_batch = tf.train.batch([paths_L, paths_depth, input_images, target_images], batch_size=a.batch_size)
...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    assertion = tf.assert_equal(tf.shape(generator_inputs)[-1], 1, message="image does not have 1 channel")
    with tf.control_dependencies([assertion]):
...
def save_images(fetches, step=None):
    ...
    for i, in_path in enumerate(fetches["pathsL"]):
        ...
        for kind in ["inputsL", "outputs", "targets"]:
